...
async function mainMenu() {
    ...
    if (menu === "5. Show Student Status")
        await showStatus();
    if (menu === "6. End Menu") {
        console.log(chalk.bgBlue.white(` Thank you for using Student Management System\n`));
        process.exit();
    }
    mainMenu();
}
...
async function addNewStudent() {
    ...
    console.log(`Student ${student.getName()} add with ID ${student.getStudentID()}\n`);
}
...
async function selectStudent() {
    if (students.length === 0) {
        ...
        return (students.find((std) => std.generateStudentID() === stdSelect.stdID) ||
            null);
    }
}
// selectStudent() start here
